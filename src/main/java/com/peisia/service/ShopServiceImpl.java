...
public class ShopServiceImpl implements ShopService{
	...
	public void buyDice(String id) {
		...
		mapper.buyDice(id); //다이스박스가 1개 추가되어 디비에 저장되는 함수(매퍼 거쳐서)
		mapper.payGold(id,10000); //위 다이스박스를 1개 구매가격을 보유 골드에서 차감시키는 함수(매퍼 거쳐서)
	}

	@Override
	public WealthDto getWealth(String id) {
		return mapper.getWealth(id);
	}

	@Override
	public void payGold(String id) {
		//todo
		//나중에 여기서 사려는 상품의 가격 가져오기 등 동적처리가 되야함. (기능이 추가 될 경우. 상점 등)
		mapper.payGold(id,10000);	//현재는 다이스 구매만 가능해서 하드코딩함.
	}

	@Transactional	//트랜잭션 처리함
	@Override
	public void sellCard(long no, String id) {
		cardMapper.delCard(no);	//카드 삭제
		mapper.buyGold(3000, id); // 판매 대금 처리		
	}
}